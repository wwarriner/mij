 ...
 *         <li>Carlos Ortiz, North Carolina State University, Chapell Hill,
 *         USA.</li>
 *         <li>Jean-Yves Tinevez, Imagopole, Institut Pasteur, Paris,
 *         France.</li>
 ...
 *      <a href="http://bigwww.epfl.ch/sage/soft/mij/">http
...
public class MIJ {
	...
	/**
	 * Starts new instance of ImageJ specifying the plugins directory and macros
	 * directory. For this to work correctly, DO NOT end homeDir with plugins and DO
	 * NOT leave homeDir null. See setupPluginsAndMacrosPaths() in Menus.java and
	 * note that Menus.updateImageJMenus() is the only public function that has
	 * access to setupPluginsAndMacrosPaths().
	 *
	 * @param homeDir Location of the user's homeDir E.g. /home/user (ImageJ
	 ...
	 *
	 * @param args   String of the same format as ImageJ commandline e.g. "-debug
	 *               -ijpath /opt/ImageJ/plugins"
	 * @param IJpath String that points to the folder containing ij.jar and plugins
	 *               and macros folder
	 */
	...
	private static void launch(String myargs[]) {
		...
		if (imagej instanceof ImageJ) {
			if (verbose) {
				...
				System.out.println("ImageJ> Directory plugins: "
						+ (IJ.getDirectory("plugins") == null ? "Not specified" : IJ.getDirectory("plugins")));
				System.out.println("ImageJ> Directory macros: "
						+ (IJ.getDirectory("macros") == null ? "Not specified" : IJ.getDirectory("macros")));
				System.out.println("ImageJ> Directory luts: "
						+ (IJ.getDirectory("luts") == null ? "Not specified" : IJ.getDirectory("luts")));
				System.out.println("ImageJ> Directory image: "
						+ (IJ.getDirectory("image") == null ? "Not specified" : IJ.getDirectory("image")));
				System.out.println("ImageJ> Directory imagej: "
						+ (IJ.getDirectory("imagej") == null ? "Not specified" : IJ.getDirectory("imagej")));
				System.out.println("ImageJ> Directory startup: "
						+ (IJ.getDirectory("startup") == null ? "Not specified" : IJ.getDirectory("startup")));
				System.out.println("ImageJ> Directory home: "
						+ (IJ.getDirectory("home") == null ? "Not specified" : IJ.getDirectory("home")));
				System.out.println("--------------------------------------------------------------");
	 ...
	 *
	 * @param roiarray give coordinates or positions of the ROI depending of the ROI
	 *                 type
	 * @param type     supported types: Roi.LINE, Roi.RECTANGLE, Roi.POINT,
	 *                 Roi.OVAL, Roi.POLYLINE, Roi.POLYGON, Roi.ANGLE
	 */
	public static void setRoi(double[][] roiarray, int type) {
		ImagePlus imageplus = WindowManager.getCurrentImage();
		switch (type) {
			case Roi.LINE:
				Line roi = new Line((int) roiarray[0][0], (int) roiarray[1][0], (int) roiarray[0][1],
						(int) roiarray[1][1]);
			...
			case Roi.OVAL:
				...
				OvalRoi oval = new OvalRoi((int) roiarray[0][0], (int) roiarray[1][0], Math.abs(width),
						Math.abs(height));
	 ...
	 * short, int, float and double. The dimensionality of the 2 (image) or 3 (stack
	 * of images)
	 *
	 ...
	 *
	 * The last index of the array is the color channel index (3 channels) in the
	 * follwing order Red-Green-Blue.
	 *
	...
	/**
	 * Create a new color image in ImageJ from a Matlab variable with a specified
	 * title.
	 *
	 * @param title title of the new image
	 * @param is    Matlab variable
	...
	/**
	 * Create a new 3D color image in ImageJ from a Matlab variable with a specified
	 * title.
	 *
	 * @param title title of the new image
	 * @param is    Matlab variable
